...
/// Something a pane refused to draw, in words, with how many times it happened.
///
/// Crosses the socket so the *client* can say it, in Turn's own furniture. It deliberately
/// does not travel as cells: a refusal notice written into the grid lands at the program's
/// cursor and corrupts a layout the program will repaint without ever overwriting Turn's
/// text. Turn's voice and the program's screen are different surfaces, and this type is the
/// boundary between them.
///
/// The text is generated by Turn from a closed set of reasons and never contains anything
/// the process supplied, so a client may render it as-is. The length bound is belt and
/// braces for a peer that is not the daemon.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ImageNotice {
    /// The sentence to show, already complete and already bracketed.
    pub text: String,
    /// How many sequences this sentence stands for. One, usually.
    pub count: u32,
}

/// How many distinct refusal sentences a screen may carry.
pub const MAX_IMAGE_NOTICES: usize = 8;

/// The longest refusal sentence a peer may send.
///
/// The longest Turn generates is comfortably under this; the bound exists so a client's
/// chrome cannot be filled by a peer that is not the daemon.
pub const MAX_IMAGE_NOTICE_CHARS: usize = 160;

/// Refuses a notice table before it is rendered.
pub fn check_notices(notices: &[ImageNotice]) -> Result<(), ImageError> {
    if notices.len() > MAX_IMAGE_NOTICES {
        return Err(ImageError::TooManyNotices {
            count: notices.len(),
            max: MAX_IMAGE_NOTICES,
        });
    }
    for notice in notices {
        let chars = notice.text.chars().count();
        if chars == 0 || chars > MAX_IMAGE_NOTICE_CHARS {
            return Err(ImageError::NoticeLength {
                chars,
                max: MAX_IMAGE_NOTICE_CHARS,
            });
        }
        if notice.count == 0 {
            return Err(ImageError::NoticeCount);
        }
    }
    Ok(())
}
...
pub enum ImageError {
    ...
    #[error("a screen may carry {max} image notices, not {count}")]
    TooManyNotices { count: usize, max: usize },
    #[error("an image notice of {chars} characters is not one a pane can show (max {max})")]
    NoticeLength { chars: usize, max: usize },
    #[error("an image notice must stand for at least one refused sequence")]
    NoticeCount,
}
