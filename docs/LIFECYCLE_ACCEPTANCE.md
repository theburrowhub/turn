...
| Recursive Groups and CheckoutScope have separate lifecycle | A depth-128 same-Session Group forest survives reverse-order restore and atomic subtree operations; self/indirect/concurrent cycles and corrupt/depth overflow refuse without a partial tree. Group promote/delete never stops work. CheckoutScope create/adopt/missing/conflicted/reconcile/unbind/remove state remains separate from optional Group binding proposed/current/stale/unbound; binding unbind changes no scope/worktree, scope unbind retains the worktree, remove alone may delete it, adopted defaults to keep, and presentation move never rewrites cwd. |
