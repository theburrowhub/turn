...
the ADR-064/065/066 capability set or completion of the operator control plane. Provider-specific observations below
...
ADR-064/065/066 add a provider-neutral acceptance layer above this historical vertical. Every dedicated adapter
...
| Native jobs | Exercise all nine exact adapter keys and wire mappings over independent definition, schedule, iteration, presence, projection and reconciliation axes. Create atomically reserves one Session/Group-owned Job Node+CreationId before dispatch; create/run-now lost receipts reconcile only by proved correlation and never duplicate. Cancel-creation works only while prepared; cancel-iteration fences one queued/running key. Local hide-activity/forget/restore preserve the deletion fence and emit zero provider request; actionable forgotten-job evidence routes through the exact AttentionId/Session/profile/target provisional view. A Turn Flow is visibly different. |
...
| WebPreview and Browser | Select an inert WebPreview Resource and prove zero script/network/navigation. In a separately created isolated Browser, exercise typed address/history and reviewed local HTML/localhost, reject origin rebinding, popup/download escalation and page/script control messages, then restore metadata with zero automatic reload. |
| Typed and remote permission | Fresh supported typed permits LocalDesktopForegroundAuthority's exact local op. Separately issue, E2EE-deliver and ack one immutable grant; exact remote op consumes it before provider effect and binary/non-binary options wait for evidence. Double-submit, revoke/expiry/reconnect/interaction/attempt/binding/capability races and crash boundaries prove one winner, terminal grant, recoverable receipt and lookup-only reconciliation. Raw bytes fail under typed; fresh supported verified-local-PTY permits only daemon-encoded desktop fallback. Degraded/unknown/stale/unsupported/none, offline replay, widening and remote/voice/hook/background fallback fail. |
...
| Automatic hierarchy arrangement | With 1,000 nested, collapsed, filtered and variable-height rows, restore and resize/zoom while injecting one topology revision. Prove the exact bounded adjacent-row-gap and virtual prefix-sum equations, non-overlap and identical logical/accessibility order, no Tidy/Arrange interaction, stable selection/focus and no layout-created hierarchy/runtime/input/Attention revision. |
...
| Bounded utility surfaces | Exercise Directory/watches, commit DAG/changed files, bounded terminal/document search, Media import/playback and plain/Markdown projection at their exact limits. Each remains the selected Node's one WorkSurface, reports gaps/staleness, performs no implicit input/network/control and preserves terminal-input/Attention latency. |
| Repository proposal and transfer | Use separate RepositoryBackend/WorkItemSource host grants; run executable and broker proposal profiles through sandbox canaries/limits; transfer across two independently authorised targets and a client stream. Lost replies and crash boundaries reconcile once, primary `main` remains read-only and no proposal/transfer commits, pushes, overwrites or executes content. |
| Catalogue, signed feeds and update | Prove toolbar/palette/menu/shortcut identity through one CommandCatalogue. Mutation fixtures for all five signing domains reject noncanonical bytes, wrong store/domain, rotation/revocation/replay and package/manifest substitution. Announcements remain inert; update stage/apply is explicit and preserves live daemon/PTys. |
| Activity and presentation history | Ingest an ordered 10,000-event WorkItem stream with echo dedup/gap recovery, then exercise all eight presentation-only undo/redo operations, including constant-size expand-all, under owner/surface revisions. No history record can encode or replay domain/runtime/provider/SCM/input/context/Attention/credential/destructive authority. |
...
passing evidence.
The separate specification run must also pass the frozen 112-capability/185-requirement ledger, selector-v1 source census and their
