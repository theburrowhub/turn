...
packaged VoiceOver and Orca record on every advertised desktop, full-remote and Companion surface. A pass for
one row cannot approve another, and an unsupported capability must expose its reason rather than disappearing.

| Capability | Required semantic, keyboard and focus oracle |
| --- | --- |
| Foreground Session activation (`ACP-LIF-009`) | Selecting the canonical Session once announces selection and the resolved current/default child once, then attaches or starts it with no secondary action or generic “start pane” control. Existing runtime, empty/default-Shell, preflight refusal and uncertain launch are distinct status values. Expansion, preview, restore, reconnect and Attention navigation never announce or cause a launch. A refusal keeps focus on the selected Session, exposes the exact bottom-status remediation and performs no surprise focus jump; a successful asynchronous start transfers terminal focus only when the selected WorkSurface's exact attempt becomes input-ready. |
