...
pub fn close_random_fds() {
    close_random_fds_except(&[]);
}

/// Close unrelated descriptors while retaining the small set the caller explicitly
/// carries into the child. This keeps the original leak-prevention default intact.
fn close_random_fds_except(preserved_fds: &[libc::c_int]) {
...
impl PtyFd {
    ...
    fn spawn_command(&self, builder: CommandBuilder) -> anyhow::Result<std::process::Child> {
        ...
        let preserved_fds = builder.preserved_fds.clone();
